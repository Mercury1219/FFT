...
function plotNatureFFTRidge3D(fftData, maxFreq, plotSettings)
    ...
    xLimitMax = maxFreq + depthLengthX + 0.35 * maxFreq;
    yLimitMax = zMax + depthLengthY + 0.10 * zMax;
    ...
    ylabel(ax, "FFT 幅值", "FontName", "SimSun", "FontSize", 18, "FontWeight", "bold");
    xticks(ax, 0:plotSettings.frequencyTickStep:maxFreq);
    yticks(ax, linspace(0, zMax, 6));
    yticklabels(ax, arrayfun(@(v) sprintf("%.0f", v), linspace(0, zMax, 6), ...
        "UniformOutput", false));
...
function drawObliqueDepthAxis(ax, maxFreq, depthLengthX, depthLengthY, rollFreqs, axisLabel)
    ...
    for i = 1:nLayer
        ...
        text(ax, x + xTextOffset, y, ...
            sprintf("%.2f", rollFreqs(i)), ...
            "FontName", "Times New Roman", ...
            "FontSize", 15, ...
    ...
    text(ax, maxFreq + depthLengthX + 1.25 * xTextOffset, depthLengthY + 0.12 * depthLengthY, ...
        axisLabel, ...
        "FontName", "SimSun", ...
        "FontSize", 15, ...
        "FontWeight", "bold", ...
